...
| v0.7 | 2026-09-12 | S63 收尾：BUG-004 ik 中文分词修复（analysis-ik 8.13.0 安装持久化 + 索引升版 inspection_alarm_v2/ik_smart + reindex 10,716 条），TC027 转绿；功能测试 34/34；v0.6 验收 8/8 | 随本修订提交 |
...
| BUG-004 | TC027 | ES 未安装 ik 中文分词插件 | 轻微 | `elasticsearch-plugin list` 输出为空 | 插件未随环境安装 | **S63 已修复**：经容器网络从 infinilabs 发布源下载 `elasticsearch-analysis-ik-8.13.0.zip`（GitHub 直链与 API 均不可用：仓库已迁移+API 限流，排错实录）→ 安装并持久化（`docker/es-plugins` bind 挂载 + `install-ik.ps1` 可复现）→ 索引升版 `inspection_alarm_v2`（description=ik_smart）+ `_reindex` 迁移 10,716 条 | 回归通过：V-7 中文检索"人员"300 条、"红外温度"5 条；`_analyze` 分词正确 |
