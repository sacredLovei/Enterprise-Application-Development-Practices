...
| TC010 | 异常测试 | 设备离线状态更新 | 1. 无人机正在上报心跳；2. 关闭仿真进程；3. 等待超时检测 | 平台识别心跳超时，设备状态更新为离线，数据库同步更新 | 15 s 内状态变为 OFFLINE，并生成 `DEVICE_OFFLINE` 告警 | 手动下线后 18 s 内 OFFLINE + DEVICE_OFFLINE 告警；手动上线 40 s 内恢复 ONLINE | 通过 |
