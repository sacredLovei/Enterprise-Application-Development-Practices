...
关键实测指标：检索 P95 **28 ms**（目标 500 ms）、网关吞吐 **2,632 rps**（目标 200 QPS）、遥测批量零丢失、限流与故障转移生效（第 6 章）。测试期间共登记缺陷 8 项（BUG-001~008），修复 6 项、设计差异 2 项，其中"设备列表 888.8 ms → 复合索引 36.3 ms"的优化路径完整演示了"先测量、再定位、后优化、再复测"的方法论。
...
**实测缺陷登记（S50 + v0.6 全量回归，2026-09-12）**——按表 6-4 模板逐项登记，共 8 项：修复 6 项（BUG-001/003/004/005/006/007）、遗留 0 项、设计差异 2 项（BUG-002/008）。
...
| 2 | 影像元数据链路未接通（BUG-008 设计差异，`inspection.image.meta` 主题为空） | 设计的"图片元数据入库"环节缺失 | S72：仿真产生元数据 + 消费入库 + 查询接口 |
