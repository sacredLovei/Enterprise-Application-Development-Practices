...
**本版本（v0.8）对应提交号**：随本修订提交产生（`git log` HEAD）
...
第 5.4 节系统运行效果截图仍为 `【待回填】`（截图需按 `docs/截图操作清单.md` 逐图补拍，图 5-1~5-16）；第 6 章测试实际结果、缺陷登记与测试结论已于 2026-09-12 S50 执行后据实回填，v0.6 迭代（S61~S64）的回归结论已同步——课程明确要求"如实记录项目遇到的问题，不虚构测试结果"，本章所有数字均可由 `docker/init/accept-s50-*.ps1`、`accept-v06.ps1` 与 `test/jmeter/` 下资产复现。
...
> 唯一未实现项 IT001 为设计差异（设备注册由心跳 upsert 驱动，无独立注册端点，已登记 BUG-002）。v0.6 迭代将 S50 登记的两项遗留（BUG-003 复核派单、BUG-004 ik 分词）全部实现并回归通过；不通过数全程为 0。
