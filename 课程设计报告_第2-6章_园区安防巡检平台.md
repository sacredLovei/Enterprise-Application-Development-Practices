...
**技术栈基线**：Docker · Hadoop HDFS 3.3.6 · MongoDB 6.0 · Apache Kafka 3.6（KRaft）· Elasticsearch 8.13 + Kibana · Nginx 1.25 · Spring Boot 3.2（Java 17）· Vue 3
...
第 5.4 节系统运行效果截图、第 6.3~6.6 节测试实际结果与测试结论均为 `【待回填】`——课程明确要求"如实记录项目遇到的问题，不虚构测试结果"，本文件不预填任何测试通过状态。
...
- **镜像成熟**：Kafka 3.x 起支持 **KRaft 模式**（去掉 ZooKeeper），Docker 单容器即可起完整集群，比 RocketMQ 的 NameServer + Broker + 可选 ZooKeeper 三件套更省资源、故障面更小。
...
| 消息中间件 | Apache Kafka（KRaft） | 3.6 | 备选 RocketMQ：需 NameServer+Broker 多进程，单机资源占用高，本项目无事务消息需求，**否** |
| 检索引擎 | Elasticsearch + Kibana | 8.13 | 备选 Solr：生态与可视化弱于 ES，**否** |
| 网关 | Nginx | 1.25 | 备选 Spring Cloud Gateway：需引入微服务体系，超出范围，**否** |
| 后端 | Spring Boot + Java | 3.2 / JDK 17 | 备选 Python FastAPI：开发更快但 Java 系生态与本课资料契合度更高，且需与 Kafka/Hadoop 客户端生态匹配，**否** |
...
| `kafka` | `bitnami/kafka:3.6` | 9092（内）/ 9093（控制器）/ 9094（外） | 9094 | KRaft 模式，无 ZooKeeper |
...
| `backend-1/2` | 自构建 `inspection-backend:1.0` | 8081 / 8082 | 不暴露 | 无状态双实例，演示负载均衡 |
| `*-sim-*` | 自构建 `inspection-simulator:1.0` | — | 不暴露 | 2 机 + 2 狗，纯生产者/消费者 |
...
| JDK | **17**（LTS） | Spring Boot 3.x 最低要求 17 |
| Maven | 3.9.x | 依赖管理 |
...
public class UavSimulator {
    ...
    @KafkaListener(topics = TopicConst.TASK_COMMAND, groupId = "sim-uav")
...
**【待回填】实施后须补充**：① 每条问题的真实日志片段与截图；② 实际耗时（用于 3.4.3 进度复盘）；③ 小组内"谁排查的、怎么定位的"——这是个人自我总结报告的必要素材。
...
> **本章状态说明**：测试环境、测试目标、用例设计、指标口径为**已确定的设计成果**；"实际结果"与"测试状态"两列须在系统实际运行后据实填写。按课程学术诚信要求，本文件**不预填任何通过状态**。
...
| 组件版本 | 同表 5-1（HDFS 3.3.6、MongoDB 6.0、Kafka 3.6、ES 8.13、Nginx 1.25） |
...
|---|---|---|---|---|---|---|---|
| TC001 | 设备仿真模块 | 无人机设备注册上线 | 1. 仿真模块已启动；2. 网络正常 | 1. 启动无人机仿真实例；2. 调用设备注册接口；3. 查看平台设备台账页面 | 1. 无人机成功注册；2. 状态显示在线；3. 设备编号与类型正确 | 【待执行】 | 【待执行】 |
| TC002 | 设备仿真模块 | 机器狗心跳上报 | 1. 机器狗仿真已注册上线；2. 消息中间件正常 | 1. 机器狗定时发送心跳；2. 消费端消费心跳；3. 查看数据库设备状态记录 | 定时收到心跳（5 s 一次），`lastHeartbeat` 持续更新，状态保持在线 | 【待执行】 | 【待执行】 |
| TC003 | 消息中间件 | 无人机上报巡检数据消息生产消费 | 1. Kafka 服务正常、topic 已创建；2. 生产者与消费者已启动 | 1. 无人机产生一条 GPS + 图片元数据消息并发送；2. 观察消费者控制台；3. 查看消息持久化 | 消息成功生产；消费者正常消费；消息持久化保存无丢失 | 【待执行】 | 【待执行】 |
| TC004 | 消息中间件 | 平台下发巡检任务指令给机器狗 | 1. 设备仿真在线；2. 消息中间件正常 | 1. 平台下发巡检任务指令；2. 仿真设备消费指令 topic；3. 仿真设备打印任务内容 | 机器狗成功接收指令，内容与下发一致 | 【待执行】 | 【待执行】 |
| TC005 | 分布式存储 HDFS | 巡检图片元数据文件上传 HDFS | 1. HDFS 集群正常；2. 客户端连接正常 | 1. 上传一张模拟巡检图片；2. 调用 HDFS 查询接口；3. 执行下载验证 | 上传成功；可查询到文件元信息；可正常下载 | 【待执行】 | 【待执行】 |
| TC006 | 分布式数据库 | 保存设备任务记录 | MongoDB 服务正常 | 1. 平台下发一条巡检任务；2. 任务信息写入数据库；3. 按任务编号查询 | 任务记录完整入库，查询返回正确信息 | 【待执行】 | 【待执行】 |
| TC007 | Elasticsearch 检索 | 按告警类型检索巡检告警事件 | 1. ES 服务正常；2. 已写入若干告警测试数据 | 1. 前端选择告警类型=设备过热；2. 执行检索 | 正确返回全部设备过热告警，结果与入库数据一致 | 【待执行】 | 【待执行】 |
| TC008 | Nginx 网关反向代理访问 | 通过 Nginx 访问后端业务接口 | 1. Nginx 已配置反向代理；2. 后端服务正常 | 1. 客户端访问 Nginx 对外端口请求后端接口；2. 观察请求转发 | Nginx 成功转发，客户端拿到后端返回数据 | 【待执行】 | 【待执行】 |
| TC009 | Web 可视化 | 查看在线设备列表 | 1. 多台仿真设备在线；2. 前后端与 Nginx 正常 | 1. 打开设备管理页面；2. 刷新页面 | 页面正确展示全部在线设备，含编号、状态、电量 | 【待执行】 | 【待执行】 |
| TC010 | 异常测试 | 设备离线状态更新 | 1. 无人机正在上报心跳；2. 关闭仿真进程；3. 等待超时检测 | 平台识别心跳超时，设备状态更新为离线，数据库同步更新 | 15 s 内状态变为 OFFLINE，并生成 `DEVICE_OFFLINE` 告警 | 【待执行】 | 【待执行】 |
| TC011 | 设备仿真模块 | 同一设备消息有序性验证 | Kafka 正常，无人机在线 | 1. 连续下发 5 条指令给同一机器狗；2. 查看消费顺序 | 5 条指令按下发顺序被消费（验证同 key 进同一分区） | 【待执行】 | 【待执行】 |
| TC012 | 设备仿真模块 | 多设备并发在线 | 已启动 2 机 + 2 狗 | 同时启动全部仿真实例并观察 10 分钟 | 4 台设备均保持在线，互不干扰，遥测正常 | 【待执行】 | 【待执行】 |
| TC013 | 设备仿真模块 | 故障注入——电量骤降 | 无人机在线 | 调用 `POST /sim/fault type=BATTERY_DROP` | 电量下降至阈值以下并触发 `BATTERY_LOW` 告警 | 【待执行】 | 【待执行】 |
| TC014 | 设备仿真模块 | 设备重复注册幂等 | 设备已注册 | 用相同 `deviceId` 再次注册 | 不产生重复设备记录，返回原设备信息 | 【待执行】 | 【待执行】 |
| TC015 | 消息中间件 | 消费失败进死信主题 | Kafka 正常 | 构造一条无法解析的消息并发送至告警主题 | 重试 3 次后进入 `inspection.dlq`，不阻塞后续消息消费 | 【待执行】 | 【待执行】 |
| TC016 | 消息中间件 | 消费幂等——重复消息不重复入库 | 已存在某 `alarmId` 告警 | 同一条告警消息重复发送 3 次 | 数据库与 ES 中该告警记录数仍为 1 | 【待执行】 | 【待执行】 |
| TC017 | 消息中间件 | 消费者重启续消费 | 已消费部分消息 | 1. 停止消费者；2. 生产 1,000 条消息；3. 重启消费者 | 消费者从上次 offset 继续，总消费条数与生产条数一致 | 【待执行】 | 【待执行】 |
| TC018 | 分布式存储 HDFS | 文件下载与 MD5 校验 | 已上传图片并有元数据 | 1. 按路径下载图片；2. 计算 MD5 与元数据比对 | MD5 完全一致 | 【待执行】 | 【待执行】 |
| TC019 | 分布式存储 HDFS | 目录分区规范验证 | 已上传多张图片 | 查看 HDFS 目录结构 | 路径符合 `/inspection/{type}/{yyyy}/{MM}/{dd}/{deviceId}/` 规范 | 【待执行】 | 【待执行】 |
| TC020 | 分布式数据库 | 遥测数据 TTL 过期 | `device_status` 有历史数据 | 查看集合索引定义 | 存在 TTL 索引且 `expireAfterSeconds` 为 2,592,000（30 天） | 【待执行】 | 【待执行】 |
| TC021 | 分布式数据库 | 就近派单选中最近机器狗 | 2 台机器狗在线且空闲，位置不同 | 在靠近机器狗 A 的位置产生告警，触发派单 | 选中机器狗 A（2dsphere 查询生效） | 【待执行】 | 【待执行】 |
| TC022 | 分布式数据库 | 任务状态并发流转 | 任务处于 `DISPATCHED` | 并发发起两次取消请求 | 仅一次成功，状态不出现非法流转 | 【待执行】 | 【待执行】 |
| TC023 | Elasticsearch 检索 | 按地理半径检索告警 | ES 有含坐标的告警数据 | 以某坐标为中心检索 500 m 内告警 | 仅返回范围内告警，边界值处理正确 | 【待执行】 | 【待执行】 |
| TC024 | Elasticsearch 检索 | 按时间范围检索 | ES 有跨多日数据 | 检索指定时间区间告警 | 返回结果全部落在区间内，时间排序正确 | 【待执行】 | 【待执行】 |
| TC025 | Elasticsearch 检索 | 组合条件检索 | ES 有充足数据 | 同时指定类型 + 等级 + 时间 + 地理范围 | 返回结果满足全部条件（AND 语义） | 【待执行】 | 【待执行】 |
| TC026 | Elasticsearch 检索 | 统计聚合正确性 | ES 有 10,000 条告警 | 调用统计接口，与 MongoDB 直接聚合结果比对 | 类型分布与时间趋势数值与数据库统计一致 | 【待执行】 | 【待执行】 |
| TC027 | Elasticsearch 检索 | 中文分词检索 | 已安装 ik 插件 | 检索中文关键词（如"围墙""人员"） | 返回包含该词的告警记录 | 【待执行】 | 【待执行】 |
| TC028 | Nginx 网关 | 负载均衡分发验证 | 双后端实例正常运行 | 连续请求 `/api/devices` 20 次，记录响应头实例号 | 两个实例均被分发到，比例接近均衡 | 【待执行】 | 【待执行】 |
| TC029 | Nginx 网关 | 单实例故障转移 | 双实例正常 | 1. 停止 backend-1；2. 持续请求接口 | 请求全部由 backend-2 处理，无 502 或仅瞬时失败 | 【待执行】 | 【待执行】 |
| TC030 | Nginx 网关 | 限流生效 | Nginx 限流已配置 | 以超过 20 r/s 的速率请求 API | 超出部分返回 503，前端展示"请求过于频繁" | 【待执行】 | 【待执行】 |
| TC031 | Nginx 网关 | 前端 History 路由刷新 | 前端已部署 | 直接访问 `/alarms` 并刷新页面 | 正常返回告警页，不出现 404 | 【待执行】 | 【待执行】 |
| TC032 | Web 可视化 | 告警详情证据链展示 | 存在含复核结果的告警 | 打开告警详情 | 同时展示高空原图与机器狗红外复核图及结论 | 【待执行】 | 【待执行】 |
| TC033 | Web 可视化 | 任务下发全流程 | 设备在线 | 1. 前端创建任务并下发；2. 观察任务状态与日志 | 状态依次流转 `CREATED→DISPATCHED→RUNNING→DONE`，日志完整 | 【待执行】 | 【待执行】 |
| TC034 | Web 可视化 | 设备地图坐标正确性 | 设备在线 | 查看地图上设备位置 | 位置落在园区范围内，无坐标颠倒现象 | 【待执行】 | 【待执行】 |
...
**【待回填】**：实施后按上表登记真实缺陷。这是报告"关键问题与解决方案"（5.3 节）的实证来源，也是答辩"你遇到过什么问题、怎么排查的"的直接答案。
...
#### 6.6.1 结果汇总（表 6-7）【待回填】

| 用例组 | 用例数 | 通过 | 不通过 | 通过率 |
|---|---|---|---|---|
| 功能测试（TC001~TC034） | 34 | 【待回填】 | 【待回填】 | 【待回填】 |
| 接口测试（IT001~IT015） | 15 | 【待回填】 | 【待回填】 | 【待回填】 |
| 性能测试（PT001~PT007） | 7 | 【待回填】 | 【待回填】 | 【待回填】 |

#### 6.6.2 性能指标实测（表 6-8）【待回填】

| 指标 | 目标值 | 实测值 | 结论 |
|---|---|---|---|
| 遥测端到端时延 P95 | < 1.5 s | 【待回填】 | 【待回填】 |
| 检索接口 P95 | < 500 ms | 【待回填】 | 【待回填】 |
| 检索接口吞吐 | — | 【待回填】 | 【待回填】 |
| 持续上报能力 | ≥ 50 msg/s | 【待回填】 | 【待回填】 |
| 网关吞吐 | ≥ 200 QPS | 【待回填】 | 【待回填】 |
| 消息丢失数 | 0 | 【待回填】 | 【待回填】 |
| MongoDB 与 ES 记录数偏差 | 0 | 【待回填】 | 【待回填】 |

#### 6.6.3 结果分析要点（供回填时展开）

回填时不要只贴数字，须回答四个问题：

1. **是否达标**：逐条对照表 6-8 目标值给出结论，未达标项要说明差多少。
2. **瓶颈在哪一层**：通过分层排除法定位。例如检索接口 P95 超标的排查顺序是——先看 ES 慢查询日志与大查询耗时，再看后端是否在结果集上做了额外转换，最后看 Nginx 是否成为瓶颈。**不要凭直觉猜，要用数据分层排除**。
3. **与设计的偏差原因**：若实际表现优于设计预期，说明是哪项优化（见 6.7）起了作用；若劣于预期，说明哪条设计假设不成立。
4. **对架构的启示**：例如实测发现 ES 聚合是大头，则该结论可写入第 7 章"后续改进"（如引入预聚合索引或缓存）。

#### 6.6.4 测试结论【待回填】

综合功能、接口、性能三类结果，给出系统是否满足第 3 章需求与验收标准的明确结论，并列出遗留问题与影响范围。**测试结论必须与实际执行结果一致，不得为凑结论而修改数据。**
...
| O-18 | 慢查询 | 打开 ES slowlog 与 MongoDB `profiler`，按实测慢语句针对性优化 | 定位真实瓶颈而非盲目调参 | 待评估 |
| O-19 | 聚合性能 | 若统计接口为瓶颈，改用 `date_histogram` 预聚合索引或加一层结果缓存 | 聚合响应时间下降 | 待评估 |
| O-20 | 分区扩容 | 若上报吞吐接近上限，增加 topic 分区数并同步提升 `concurrency` | 吞吐线性提升 | 待评估 |
| O-21 | 图片存储 | 若图片量激增，增加缩略图与按天归档策略 | 降低 HDFS 小文件元数据压力 | 待评估 |

**优化方法论提示（答辩加分点）**：性能优化的正确顺序是"**先测量、再定位、后优化、再复测**"，而不是罗列一堆参数。O-18 之所以列为前置，正是因为**没有测量数据的调参无法证明有效**。报告中应至少给出一组"优化前 vs 优化后"的对比数据（表 6-9）。

#### 6.7.1 优化前后对比（表 6-9）【待回填】

| 优化项 | 指标 | 优化前 | 优化后 | 提升幅度 |
|---|---|---|---|---|
| O-7 BulkBulk 写入 | ES 写入 10,000 条耗时 | 【待回填】 | 【待回填】 | 【待回填】 |
| O-8 filter 上下文 | 单次组合检索 P95 | 【待回填】 | 【待回填】 | 【待回填】 |
| O-4 批量写入 | 遥测 50 msg/s 下 MongoDB CPU | 【待回填】 | 【待回填】 | 【待回填】 |
| O-10 长连接复用 | 网关吞吐 | 【待回填】 | 【待回填】 | 【待回填】 |
| O-17 日志降级 | 容器磁盘写入速率 | 【待回填】 | 【待回填】 | 【待回填】 |

### 6.8 本章小结

本章按"环境 → 目标 → 功能测试 → 接口测试 → 性能测试 → 结果分析 → 优化"的顺序构建了完整测试方案，并明确了本章的诚实性边界：测试环境、目标、用例与优化措施为已确定的设计成果，"实际结果/测试状态"两列一律留待实施后据实填写，不预填任何通过结论。测试设计上做了三件事：一是把第 3 章的量化需求（遥测时延 P95 < 1.5 s、检索 P95 < 500 ms、并发 ≥ 50 msg/s、零消息丢失等）逐条转成可判定的测试目标 G-1~G-8；二是把课程的样例用例从 10 条扩充到 34 条并建立**需求追溯矩阵**，使每条功能需求都有用例覆盖；三是区分了"设计内优化"与"压测后按瓶颈追加的优化"，强调优化必须建立在实测定位之上，并以"优化前后对比表"作为优化有效性的证据。第 2~6 章至此形成闭环：第 3 章定需求与指标，第 4 章定架构与数据，第 5 章定实现与故障预案，第 6 章验证需求达成度并反馈优化方向。
