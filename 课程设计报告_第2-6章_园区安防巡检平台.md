...
- JMeter 性能脚本（`test/jmeter/`）与 `accept-s50-*.ps1` 系列尚未适配，第 6 章历史性能数据均为**鉴权前**测量值（口径不变，如实标注）。
...
| TC021 | 分布式数据库 | 就近派单选中最近机器狗 | 2 台机器狗在线且空闲，位置不同 | 在靠近机器狗 A 的位置产生告警，触发派单 | 选中机器狗 A（2dsphere 查询生效） | v0.6 实现（BUG-003 回归）：2dsphere nearSphere 就近派单——V-2/V-2b 分别在 ROBOT-001/002 位置注入告警，均派给地理最近的机器狗；DONE 回执回填复核结论与红外图（D-21 仿真结论规则） | 通过（BUG-003 回归） |
