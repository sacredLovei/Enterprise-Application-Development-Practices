...
本章按 4.4 节的模块划分给出了九个模块的详细设计与关键实现。设备仿真模块采用"航点插值 + 噪声"的连续移动模型保证数据真实感，并以心跳 5 s、遥测 2 s 的双节奏解耦上报；消息接入模块通过统一生产者与"幂等消费 + 手动提交 + 死信兜底"的模板基类，把 Kafka 至少一次投递语义下的可靠性责任集中到一处；存储模块采用 WebHDFS 规避 Hadoop 客户端依赖冲突，并明确"图片本体进 HDFS、元数据进 MongoDB"；数据库模块给出了心跳 upsert、遥测批量写、2dsphere 就近派单、离线检测、任务状态原子流转五个关键实现；检索模块强调 `filter` 上下文与 Bulk 写入两个性能要点；网关模块给出 Nginx 配置全文及可用于现场演示负载均衡的实例标识设计；认证鉴权模块以"HMAC 自包含令牌 + 拦截器统一校验 + MongoDB 共享吊销表"实现 FR-5.7，并给出 401/403 语义边界与配置化账号的简化说明；统一异常处理模块（S89）把业务异常、参数校验失败、请求体不可解析、参数缺失/类型错误与兜底异常收敛为 `{timestamp,status,code,error,message,path[,fields]}` 一致契约，保证异常路径 4xx 而非 5xx、500 不泄露异常栈；前端模块给出了"前端不感知后端拓扑"的 API 封装、分页面刷新策略与登录态闭环（路由守卫 + 自动携带令牌 + 401 回跳）。5.3 节汇总了 17 项高频故障的现象、根因与预案，构成本章最具工程价值的部分。第 6 章将基于本章实现与第 3 章需求指标，设计并执行系统测试。
---
