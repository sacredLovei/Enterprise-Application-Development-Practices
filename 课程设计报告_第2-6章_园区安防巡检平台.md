...
**实测缺陷登记（S50 + v0.6 回归，2026-09-12）**——按表 6-4 模板逐项登记，共 8 项：修复 5 项（BUG-001/003/005/006/007）、遗留 1 项（BUG-004）、设计差异 2 项（BUG-002/008）。
...
|---|---|---|---|---|---|
| 功能测试（TC001~TC034） | 34 | 33 | 0 | 1（TC027，待 S63 ik 插件回归） | 97.1%（已实现范围 100%） |
| 接口测试（IT001~IT015） | 15 | 14 | 0 | 1（IT001 设计差异） | 93.3%（已实现范围 100%） |
| 性能测试（PT001~PT007） | 7 | 7 | 0 | 0 | 100% |

> 未实现项均有登记：IT001 为设计差异（注册由心跳驱动）；TC027 为环境受限遗留（BUG-004，ik 插件网络受限未安装，按 P-11 预案降级），S63 尝试安装后回归。BUG-003（复核派单）已在 v0.6 实现并回归通过。功能与接口测试在"已实现需求"范围内通过率均为 100%，不通过数为 0。
...
1. **是否达标**：表 6-8 七项指标全部达标，且关键指标裕量显著（检索 P95 仅为目标值的 5.6%、网关吞吐为目标值的 13 倍）。功能测试 33/34 通过、0 失败；接口测试 14/15 通过、0 失败。两个未实现项（TC027 ik 分词、IT001 注册端点）分别为环境受限遗留（S63 尝试安装）与设计差异（注册由心跳驱动），不影响已实现需求的验收。
...
系统满足第 3 章全部量化需求与验收标准：遥测端到端时延、检索 P95、持续上报零丢失、网关吞吐、负载均衡、限流、故障转移、数据一致性均实测达标（G-1~G-8 全部满足）。测试期间发现并修复 4 项真实缺陷（BUG-001 限流缺失、BUG-005 索引未落地、BUG-006 ES 总数截断、BUG-007 并发取消竞态），均为"设计有、实现缺"或"默认行为陷阱"类问题，全部回归通过；**v0.6 迭代将 BUG-003（复核派单闭环）实现并回归通过（TC021/IT009/TC032 转绿）**；遗留 1 项（BUG-004 ik 分词，S63 尝试安装）与 2 项设计差异（BUG-002、BUG-008）不阻塞验收。测试数据规模按设计补齐（10,000 条告警注入），性能结论在 10k 数据规模下成立。
...
本章完成了"环境 → 目标 → 功能测试 → 接口测试 → 性能测试 → 结果分析 → 优化"的完整闭环。功能测试 34 例（33 通过、1 未实现 TC027）、接口测试 15 例（14 通过、1 设计差异 IT001）、性能测试 7 例全部达标：检索 P95 28 ms、网关吞吐 2,632 rps、遥测批量零丢失、限流与故障转移均实测生效。测试不是"跑一遍都绿"：S50 实际暴露并修复了 4 项真实缺陷（限流缺失、时序集合缺索引、ES 总数截断、并发取消竞态），其中缺索引问题经过了"888.8 ms → 聚合方案劣化 1,229.7 ms → 复合索引 36.3 ms"的完整测量-定位-优化-复测路径，以数据驳回了想当然的优化方案；10,000 条告警的数据规模准备则验证了 6.1.1"数据过少结论无意义"的警告——ES 总数截断缺陷仅在数据超万后暴露。**v0.6 迭代兑现了 S50 登记的两项遗留之一：复核派单闭环（BUG-003）实现并经 v0.6 验收 7/7 回归通过（TC021 就近派单、IT009 复核接口、TC032 证据链展示全部转绿）**；剩余 1 项（ik 分词，BUG-004）与 2 项设计差异（注册端点、影像元数据主题）均有登记与后续计划。第 2~6 章至此闭环：第 3 章定需求与指标，第 4 章定架构与数据，第 5 章定实现与故障预案，第 6 章以实测数据验证需求达成度并反哺优化方向（6.6.3 第 4 点的架构启示将写入第 7 章后续改进）。
