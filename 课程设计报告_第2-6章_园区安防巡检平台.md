...
**文档范围**：第 1 章 绪论 → 第 7 章 总结与展望（全一册；第 2~6 章为设计阶段先行产出，第 1/7 章于 S66 补齐）
...
| v0.6 | 2026-09-12 | v0.6 迭代回归：BUG-003 复核派单闭环实现（2dsphere 就近派单 + 回执回填 review + IT009），TC021/IT009/TC032 转绿；表 6-2/6-7、6.3.3 登记、6.6.4/6.8 同步更新 | 随本修订提交 |
| v0.7 | 2026-09-12 | S63 收尾：BUG-004 ik 中文分词修复（analysis-ik 8.13.0 安装持久化 + 索引升版 inspection_alarm_v3/ik_smart + reindex 10,796 条；排错实录：v2 动态映射误建致 stats 500、ik 词典目录丢失致 NPE，见 STATE 风险 #37），TC027 转绿；功能测试 34/34；v0.6 验收 8/8 | 随本修订提交 |
| v0.8 | 2026-09-13 | S66：补齐第 1 章（绪论：背景/任务/现状/成果/结构）与第 7 章（总结与展望：工作总结、8 项不足与改进方向 S68~S72、展望）；文档升为全一册 1~7 章 | 随本修订提交 |
| v0.9 | 2026-09-21 | S88：落地 FR-5.7 简易权限认证——新增 5.2.8 认证与鉴权模块实现（HMAC 自包含令牌 + 拦截器统一校验 + MongoDB 吊销表 + 前端登录闭环）；3.2 FR-5.7 与 3.3 安全性需求口径更新；新增 TC035/TC036、IT016~IT018 与 6.6.1 汇总行；验收 13 例全过（`docker/init/accept-s88.ps1`） | 随本修订提交 |
| v1.0 | 2026-09-21 | S89：落地 5.2.9 统一异常处理与错误语义（`@RestControllerAdvice` 收敛五类异常，详见该节）；6.4.2 补统一错误响应体实测表；新增 6.4.3 单元测试（后端 14 + 仿真 6 = 20 例）与 TC037；修正 5.5 小结模块数与故障项数（八→九模块、15→17 项）；6.4.2 原文所述"全局异常处理"由本条落地为真实实现 | 随本修订提交 |
| v1.1 | 2026-09-21 | S90：测试资产适配鉴权与数据现状——全部 HTTP 脚本加登录取令牌（`docker/init/auth-helper.ps1`）、JMeter 计划以 `-Jtoken` 注入令牌并提供 `jmeter-run.ps1` 一键运行；修复 7 处陈旧断言（IT005 空闲设备选择、IT007 级联防护、IT009 由 UNIMPL 转实测并新增 IT009b、TC015 自注入坏消息、TC018 唯一临时文件 + `get` 校验、TC019 允许 `.jpg`、TC021 实环验证）；表 6-2 的 TC015/TC018/TC019/TC021 与 6.4.1 复跑结论、表 6-7 接口行按实测更新 | 随本修订提交 |
| v1.2 | 2026-09-21 | S92（用户反馈）：登录页演示账号由纯文本改为**可点击 chip 一键填入**（原提示不可点击，用户点击后表单仍为空并触发"请输入用户名与口令"校验提示）；5.2.8(6) 登录页行与图 5-17 描述同步 | 随本修订提交 |
| v1.3 | 2026-09-22 | S96~S104 前端与告警体系升级批次：5.2.7 重写为实际实现口径（Esri 灰度底图 + 园区可视化 + SSE 实时位置流），新增 (5) 界面视觉与主题体系、(6) 实时位置流、(7) 告警分类与离线分级升级、(8) 园区扩容与派单限内；5.2.1(4) 补园区与告警仿真；告警类型新增烟火告警（FIRE_SMOKE），设备运维类告警默认隐藏、异常离线过久升级严重 | 随本修订提交 |
| v1.4 | 2026-09-22 | S105 任务悬挂对账（运行期真实缺陷 BUG-009，用户报障）：回执驱动任务状态机在整机/仿真器重启后的"永久执行中"悬挂——5.2.4 新增 (6) 启动对账与超时兜底（TaskReconcileService）；5.3 预案表新增 P-18；6.3.3 补登记 BUG-009；4.4.3 任务调度职责同步 | 随本修订提交 |

**本版本（v0.9）对应提交号**：随本修订提交产生（`git log` HEAD）

> 注：文档版本号与里程碑 tag 相互独立——文档升版随本修订生效；里程碑 tag 另行打标（STATE §1）。
...
第 5.4 节系统运行效果截图仍为 `【待回填】`（截图需按 `docs/截图操作清单.md` 逐图补拍，图 5-1~图 5-17，其中图 5-17 为 S88 登录页）；第 6 章测试实际结果、缺陷登记与测试结论已于 2026-09-12 S50 执行后据实回填，v0.6 迭代（S61~S64）与课程四项要求（S85~S88：日志归档查询、多设备并发压测、备份恢复演示、简单权限认证）的实测结论已同步——课程明确要求"如实记录项目遇到的问题，不虚构测试结果"，本章所有数字均可由 `docker/init/accept-s50-*.ps1`、`accept-v06.ps1`、`accept-s86-pt8.ps1`、`accept-s88.ps1`、`restore-demo.ps1` 与 `test/jmeter/` 下资产复现。
...
园区（产业园区、科技园、大型厂区）安防的传统做法以固定摄像头 + 人工巡逻为主，长期存在三类结构性痛点：

1. **覆盖盲区大**：固定摄像头存在视角与布线盲区，周界长、角落多的园区难以全量覆盖；夜间与恶劣天气下视频有效性进一步下降；
2. **响应链条长**：发现异常（周界入侵、烟火、设备过热）后，仍需人员从值班室赶到现场确认，误报消耗大量人力，真警又常常错过黄金处置窗口；
3. **数据孤岛**：视频、门禁、传感器各自为政，告警缺乏统一汇聚、检索与统计手段，事后追溯困难。

本项目对应课程选题一"无人机-机器狗空地协同巡检集成平台"，以"**高空广域发现 + 地面抵近复核**"的空地协同模式回应上述痛点：无人机执行空中巡检，凭借广视角快速发现周界入侵、烟火等疑似事件；平台自动把**地理上最近的机器狗**派往告警点，用红外传感抵近复核、回传结论与证据图，形成"发现—派单—复核—处置"的完整闭环。系统按照《企业应用开发实践》课程要求，集成 **Hadoop HDFS、MongoDB、Kafka、Elasticsearch、Kibana、Nginx** 六大组件，实现影像分布式存储、业务数据持久化、消息异步流转、多维检索、可视化分析与统一网关接入，从而同时达成课程目标（大组件集成实践）与业务目标（可演示的园区安防闭环）。

### 1.2 课题任务与目标

**任务来源**：课程任务书选题一（推荐基础题）。要求基于六大组件构建可运行的集成平台，覆盖设备接入、消息流转、分布式存储、检索分析、网关访问与 Web 可视化，并通过系统测试验证需求达成度。

**建设目标**：

- **功能目标**（详见第 3 章 FR-1~FR-6）：无人机/机器狗仿真设备注册、心跳与遥测上报、故障注入；指令下行与任务执行回执；巡检影像入 HDFS、业务数据入 MongoDB；告警多维组合检索（类型/等级/时间/地理半径/关键词）与聚合统计；Nginx 网关（负载均衡、限流、故障转移）；Web 可视化（设备地图、任务跟踪、告警证据链、统计看板）。
- **性能与可靠性目标**（详见第 3 章指标）：遥测端到端时延 P95 < 1.5 s；检索接口 P95 < 500 ms；并发上报 ≥ 50 msg/s 零丢失；网关吞吐 ≥ 200 QPS；负载均衡与单实例故障转移生效；MongoDB 权威数据与 ES 检索副本记录数偏差为 0。

**核心业务闭环**：仿真设备心跳/遥测经 Kafka 接入 → 无人机巡检产生告警 → 平台三写（HDFS 证据图、MongoDB 权威记录、ES 检索副本）并自动派最近机器狗复核 → 机器狗回执驱动任务完成并回填复核结论与红外复核图 → 前端证据链展示与统计看板呈现。该闭环中每一步的状态流转均由消息回执驱动，是第 4~6 章设计与测试的主线。

### 1.3 国内外研究与应用现状

**无人机巡检**：以多旋翼无人机为载体的工业巡检（电力、光伏、园区安防）已进入规模化应用阶段，主流厂商产品普遍支持航线规划、自动巡航、AI 图像识别与云端任务管理。其价值点与本项目一致：高频广域覆盖、无人化作业；其局限也一致：续航短、无法抵近地面交互。

**四足机器人与地面复核**：四足机器人（机器狗）在工业场景的应用近年快速升温，具备越障能力与狭小空间通过性，可携带红外热成像、气体传感器等载荷执行厂区巡逻与定点核查。公开报道的落地场景包括变电站巡检、厂区夜间巡逻等；"无人机发现 + 机器狗复核"的协同模式是工业界正在探索的方向，本项目的仿真实现即对标该模式。

**大数据组件生态**：Hadoop HDFS 是事实标准的分布式文件系统；MongoDB 是文档型数据库代表；Kafka 是流式消息事实标准；Elasticsearch/Kibana 组合是日志与事件检索分析的主流选型；Nginx 是应用最广的反向代理。本项目将六者按"影像进 HDFS、台账进 MongoDB、消息走 Kafka、检索走 ES、分析走 Kibana、入口走 Nginx"的职责边界集成（详见第 2 章与第 4 章）。

### 1.4 主要工作与成果

本项目按"自下而上逐层验证"的路线推进（环境 → 最小链路 → 业务全量 → 前端 → 测试），主要工作与成果如下：

| 阶段 | 主要工作 | 成果（实测口径，详见第 5/6 章） |
|---|---|---|
| 环境搭建 | 六组件 Docker 编排（HDFS/Mongo/Kafka/ES/Kibana/Nginx）与初始化脚本 | 12 容器健康运行、rs0 副本集、8 个 Kafka 主题、一键可复现 |
| 最小链路 | 仿真心跳/遥测生产者 + 后端消费落库 + 网关验证 | 数据全链路打通，v0.3 里程碑 |
| 业务全量 | 告警三写、指令下行、任务回执状态机、离线检测、复核派单闭环（v0.6）、ik 中文分词（v0.6） | 4 台仿真设备（2 机 2 狗）持续在线，v0.4/v0.5 里程碑 |
| 前端可视化 | Vue3 五页面 + Leaflet 地图 + ECharts 图表 | 设备地图、任务跟踪、告警证据链、统计看板 |
| 系统测试 | TC001~TC034、IT001~IT015、PT001~PT007 全量执行 | 功能 **34/34**、接口 **14/15**（1 项为设计差异）、性能 **7/7** 全过 |

关键实测指标：检索 P95 **28 ms**（目标 500 ms）、网关吞吐 **2,632 rps**（目标 200 QPS）、遥测批量零丢失、限流与故障转移生效（第 6 章）。测试期间共登记缺陷 8 项（BUG-001~008），修复 7 项、设计差异 1 项（注册端点由心跳驱动），其中"设备列表 888.8 ms → 复合索引 36.3 ms"的优化路径完整演示了"先测量、再定位、后优化、再复测"的方法论。

### 1.5 报告组织结构

- **第 1 章 绪论**：背景、任务、现状、成果与本结构；
- **第 2 章 相关技术基础**：六组件与 Docker 的核心原理及"本项目定位"；
- **第 3 章 需求分析**：功能需求 FR-1~FR-6、非功能指标与可行性分析；
- **第 4 章 系统总体设计**：总体架构、数据设计、部署拓扑与关键技术设计；
- **第 5 章 系统详细设计与实现**：七模块详细设计与关键实现、疑难预案；
- **第 6 章 系统测试与优化**：测试方案、用例执行、结果分析与优化对比；
- **第 7 章 总结与展望**：工作总结、不足与改进方向、展望。
...
| FR-5.7 | 【加分】简易权限认证 | 登录签发 Token，Nginx 或后端做校验；本设计预留 `Authorization` 头透传与 `auth_request` 位置。**S88 已实现**：`POST /api/auth/login` 签发 HMAC 签名令牌，后端拦截器统一校验 `/api/**`（白名单仅登录接口），未认证统一 401；实现细节见 5.2.8，验收见 TC035/TC036、IT016~IT018 |
...
| | 敏感配置（口令）不硬编码进源码 | 环境变量注入（演示账号口令支持 `AUTH_ADMIN_PASSWORD` 等环境变量覆盖） |
| | 业务接口鉴权：除登录接口外全部 `/api/**` 需 Bearer 令牌，未认证返回 401 | `docker/init/accept-s88.ps1`（13 例断言，S88 实测全过） |
...
| 任务调度 `task` | 任务创建、下发指令、状态流转、日志归档、重启对账与超时兜底（S105） | `POST /api/tasks`、`GET /api/tasks`、`POST /api/tasks/{id}/cancel` |
...
| 认证鉴权 `auth`（S88） | 登录签发令牌、令牌校验、登出吊销、当前用户查询 | `POST /api/auth/login`、`GET /api/auth/me`、`POST /api/auth/logout` |
...
- **路径规范**：`/inspection/{imageType}/{yyyy}/{MM}/{dd}/{deviceId}/{uuid}.{ext}`，其中 `imageType ∈ {uav_patrol, dog_infrared, alarm_snapshot, manual_review}`（`manual_review` 为 S75 新增：人工复核上传的现场照片）。
...
仿真园区为**约 425m × 380m 的大型工业园区**：无人机巡逻环线为 8 航点（含内凹路口的园区道路），基地位于园区西南内侧（返航目的地），加工车间位于园区中部（烟火高发点位）；机器狗地面路网为 4×3 共 12 节点（格距约 120m），BFS 求最短路径。告警仿真包含：无人机巡逻以约 5% 概率发现周界入侵、以约 3% 概率于加工车间位置发现**烟火**（FIRE_SMOKE，CRITICAL）；机器狗红外测温支持 OVERHEAT 注入；电量单调下降归零后自动断电返航回充（BATTERY_LOW）。设备离线告警采用**分级升级**：心跳丢失 15 秒产生 WARN 级告警（设备运维类），持续离线超过 3 分钟升级为 CRITICAL（安防口径进入主页），设备恢复上线后告警自动关闭。
...
**(6) 回执驱动状态机的悬挂风险与对账兜底（S105）**

任务状态机是"回执驱动"的：`COMMAND_RECEIVED → RUNNING`、`DONE → DONE`，状态持久于 MongoDB；而任务的**真实执行态**（当前任务 `currentTaskId`、优先级队列 `taskQueue`）只存在于仿真器 JVM 内存。这一"持久层有状态、执行器无痕"的不对称在**整机或仿真器容器重启**时暴露：Kafka 消费组 offset 已提交、`task.command` 旧指令不会重放，仿真器重启后回到默认园区巡逻（轨迹模式初始即 PATROL），遗留的 `DISPATCHED/RUNNING` 任务永远等不到下一条回执——任务列表出现"永久执行中"的幽灵任务（实测存量一条悬挂 4 小时以上）。仅后端重启不受影响：`task.log` 回执在 Kafka 有保留，后端恢复后照常消费。

修复采用**启动对账 + 超时兜底**两层机制（`TaskReconcileService`，均为幂等条件更新，双实例并发安全）：

```java
/** 启动对账：应用就绪即清算上次运行遗留的未终结任务（宽限期内派发的不动） */
@EventListener(ApplicationReadyEvent.class)
public void reconcileOnBoot() {
    Instant cutoff = Instant.now().minusSeconds(reconcileGraceSeconds);   // task.reconcile-grace-seconds，默认 60
    abortOpenTasks(cutoff, "系统重启中断");
}

/** 超时兜底：每 60 秒扫描执行超过阈值仍未终结的任务（覆盖宽限窗口残余、单实例滚动重启、回执丢失） */
@Scheduled(fixedDelay = 60_000)
public void sweepStale() {
    abortOpenTasks(Instant.now().minus(staleMinutes, ChronoUnit.MINUTES), "执行超时自动终结");   // task.stale-minutes，默认 30
}
```

`abortOpenTasks` 仅当任务仍处于 `DISPATCHED/RUNNING` 时才条件更新为 `FAILED` 并回填 `finishTime`，同时补一条 `FAILED` 回执日志，使任务详情时间线可见终结时刻。设计取舍：单后端滚动重启时，正在执行且已过宽限期的个别任务也会被如实标记为失败（设备后续 `DONE` 回执因状态已终态不生效，与 BUG-007 取消受理同口径）——**宁可如实标失败，不留永久"执行中"的悬挂**。
...
> **v1.3 实现口径说明**：上图代码为设计阶段示意。实际实现中底图采用 **Esri 灰度瓦片**（低饱和、支持亮/暗主题自动切换，弃用 OSM 彩色瓦片以突出告警元素）；地图上叠加**园区边界（蓝色虚线多边形）**、基地与加工车间标记；设备位置由 SSE 实时推送（见下文 (6)），告警点带脉冲圈动画。

**(5) 界面视觉与主题体系（设计令牌 + 双主题 + 动效）**

前端全部颜色、圆角、阴影收敛到设计令牌文件 styles/tokens.css（CSS 自定义属性），组件只引用变量不写死颜色；图标统一为内联 SVG 组件（替换早期 emoji 字符，跨平台渲染一致）。基于令牌实现**亮色 / 暗色双主题**：通过根元素 data-theme 属性切换令牌集，侧边栏与登录页右上角均可切换，切换采用 View Transitions API 的**圆形扩散过渡**（亮色自屏幕中心向外扩散、暗色向内收缩）。页面布局去除宽度上限满幅铺开，任务管理与告警中心提供**二级子项导航**（下发任务/任务列表、告警列表/告警详情），子页间方向感知水平滑动，跨模块导航为淡入淡出。登录页包含 Canvas 粒子网络背景（粒子随光标产生引力响应）、指挥雷达动效面板（扫描扇面、航迹光点）与登录卡入场动效，全部动效在 prefers-reduced-motion 下自动降级。

**(6) 设备实时位置流（SSE + 插值动画）**

设备位置由 3 秒轮询升级为 **SSE（Server-Sent Events）1 秒推送**：后端新增 GET /api/stream/positions 端点，共享调度器每秒比对最近遥测，位置有变化才推送（15 秒心跳注释防代理空闲断连）；Nginx 为 /api/stream/ 单独配置 proxy_buffering off 与长读超时。前端不使用 EventSource（无法携带 Authorization 头），而是以 fetch 流式读取并手动解析事件帧，断线 5 秒自动重连、期间回退 3 秒轮询兜底并在图例区明示当前链路。地图标记采用**增量更新 + requestAnimationFrame 位置插值**（约 950ms 缓动），替代整层重建的跳点式刷新。

**(7) 告警分类与离线分级升级**

告警按业务语义分为两类（以告警类型静态映射派生，不新增存储字段，存量数据天然兼容）：

| 分类 | 类型 | 展示策略 |
|---|---|---|
| 安防类（SECURITY） | 周界入侵、**烟火告警（FIRE_SMOKE，新增）** | 默认显示 |
| 设备运维类（DEVICE） | 设备过热、电量不足、设备离线 | 默认隐藏，查询时可选 |

检索接口新增 category 参数（SECURITY / DEVICE / IMPORTANT）；主页口径为 IMPORTANT = 安防类全部 + 设备类中已升级 CRITICAL 且未处置的告警。**离线分级升级**解决"设备一没电就弹出多条无效告警"的问题：心跳丢失 15 秒仅产生 WARN 级设备离线告警（主页不显示）；持续离线超过阈值（默认 3 分钟，可配置）仍未恢复，才升级为 CRITICAL 进入主页；设备恢复上线后，未终态的离线告警自动置 RESOLVED 并在描述中记录关闭原因。升级与恢复均为幂等条件更新，双实例并发安全。

**(8) 园区扩容与派单限内**

园区由约 120m 的演示矩形扩容为**约 425m × 380m 的大型工业园区**：巡逻环线为 8 航点含内凹路口的园区道路，基地位于园区西南内侧，加工车间（烟火告警点位）位于园区中部；机器狗地面路网同步扩为 4×3（12 节点，格距约 120m）。任务下发地图上叠加园区边界图层，**选点型任务（定点复核/区域覆盖）的目标坐标在后端做园区边界校验**——越界返回 400"目标位置必须在园区范围内，不可外派"，前端选点图同样拦截越界点击并提示。烟火告警由无人机巡逻扫描以约 3% 概率于加工车间位置发现，走完整的"告警三写 → 就近机器狗复核 → 证据链回填"闭环。

#### 5.2.8 认证与鉴权模块实现（S88 · FR-5.7）

**(1) 设计定位**

FR-5.7（加分项"简易权限认证"）的落地。设计上预留了两种校验位置（Nginx `auth_request` 或后端拦截器），实现选择**后端统一拦截**：理由是令牌语义（有效期、吊销、角色）属于业务域，放在后端才便于与任务/告警等业务权限衔接，而 Nginx 只做 `Authorization` 头透传，保持网关职责单一。

**(2) 令牌方案：自包含 + HMAC 签名，零第三方依赖**

```
token = base64url(payloadJson) + "." + base64url(HMAC-SHA256(payloadJson, secret))
payload = { sub: 用户名, name: 显示名, role: 角色, jti: 令牌编号, exp: 过期毫秒时间戳 }
```

| 设计点 | 取舍与理由 |
|---|---|
| 不引入 JWT 库 | 只用 JDK 的 `javax.crypto.Mac`，规避镜像构建期的依赖下载（本项目构建环境需代理，见 P-1 同族问题）；令牌格式语义与 JWT 一致，后续可平滑替换 |
| 自包含令牌 | 后端任一实例凭同一密钥即可校验，天然适配 Nginx 双实例负载均衡（D-4），无需共享会话存储 |
| 签名比对 | `MessageDigest.isEqual` 常量时间比较，避免签名逐字节比较的时序侧信道 |
| 密钥管理 | `app.auth.secret`（环境变量 `AUTH_SECRET` 覆盖）；**双实例必须一致**，否则跨实例校验必失败——这是本方案唯一的部署约束 |

**(3) 鉴权链路（`AuthInterceptor` + `WebConfig`）**

拦截 `/api/**`，白名单仅 `/api/auth/login`；`/actuator/health`（容器健康探针）、`/swagger-ui/**`、`/v3/api-docs`（S76 接口文档）因不在 `/api/**` 前缀内天然放行，跨域预检 `OPTIONS` 亦放行。校验次序：

```
Bearer 头存在？ → 签名与有效期通过？ → jti 未在吊销表中？
   否 ↓               否 ↓                    是 ↓
        401（缺少访问令牌）  401（令牌无效或已过期）   401（令牌已登出）
```

失败统一返回 **401** 与结构化 JSON（`code`/`error`/`message`），三类失败语义可区分但不泄露内部细节。**401 与 403 的语义边界**：本步只实现"是否认证"（401）；基于角色的细粒度授权（如"仅 ADMIN 可执行维护接口"，403）未实现，属如实记录的简化。

**(4) 登出与吊销表（为什么必须落库）**

无状态令牌的固有问题是**无法主动作废**。登出时把该令牌的 `jti` 写入 MongoDB 集合 `auth_revoked`（字段 `_id=jti`、`username`、`revokedAt`、`exp`，其中 `exp` 建 **TTL 索引**过期自动清理），拦截器每请求按主键查一次（索引命中，代价极小）。

> **关键取舍**：吊销表不能放单实例内存。后端为双实例负载均衡（D-4），内存黑名单会出现"在 backend-1 登出、下一次请求落到 backend-2 仍然有效"的错误——这是本模块唯一必须依赖分布式存储的地方。

**(5) 账号与口径**

| 项 | 值 |
|---|---|
| 演示账号 | `admin` / `admin123`（ADMIN · 系统管理员）、`operator` / `operator123`（OPERATOR · 巡检值班员） |
| 口令来源 | 配置文件 `app.auth.users`，支持 `AUTH_ADMIN_PASSWORD` / `AUTH_OPERATOR_PASSWORD` 环境变量覆盖（不硬编码进源码，对应 3.3 安全性需求） |
| 令牌有效期 | 12 小时（`AUTH_TTL_HOURS` 可配） |
| 接口 | `POST /api/auth/login`（唯一免鉴权）、`GET /api/auth/me`、`POST /api/auth/logout` |

已知简化（如实记录）：账号为配置化而非用户表，口令为明文比对而非加盐哈希；生产应改为用户表 + bcrypt/argon2，并增加登录失败次数限制与审计。

**(6) 前端登录闭环**

| 环节 | 实现 |
|---|---|
| 登录页 | 新增 `/login` 路由（独立全屏布局，无侧边栏），表单校验 + 错误提示 + 加载态；**S92 起演示账号以可点击 chip 呈现，点击即自动填入用户名与口令**（此前为纯文本提示，用户点击无反应而误以为账号失效） |
| 登录态存储 | `localStorage`（`inspection_token` / `inspection_user`），登录响应的用户名、显示名、角色、过期时间一并保存 |
| 自动携带 | axios 请求拦截器统一注入 `Authorization: Bearer <token>`——业务页面代码零改动 |
| 401 处理 | 响应拦截器捕获 401 → 清空本地登录态 → 跳转 `/login?redirect=<原地址>`（回跳），已在登录页则不重复跳转 |
| 路由守卫 | 未登录访问任意业务路由 → 重定向登录页并带 `redirect`；已登录访问 `/login` → 直接进首页 |
| 退出登录 | 侧边栏"退出登录"先调用后端吊销接口，再清本地登录态并回登录页；侧边栏同时展示当前用户（👤 显示名 · 角色） |

**(7) 实测结果（`docker/init/accept-s88.ps1`，13 例 13 通过）**

| 断言 | 场景 | 实测 |
|---|---|---|
| A-1 / A-2 / A-3 | 无令牌访问、错误口令、空口令 | 401 / 401 / 400（异常路径无 500） |
...
| A-6 / A-7 | 篡改令牌、伪造签名 | 401 / 401 |
| A-8 / A-9 | `/api/auth/me`、第二账号登录 | 200（`user=admin`）/ 200（`role=OPERATOR`） |
| A-10 | 同一令牌跨双实例 | 16 次请求命中 `backend-1` **与** `backend-2`，全部 200 |
| A-11 | 接口文档白名单 | `/v3/api-docs` 无令牌 200 |
| A-12 | 登出吊销 | 登出 200 → `auth_revoked` 文档 0→1 → 旧令牌复用 401 |
| A-13 | 前端产物 | 构建产物含登录页与令牌处理代码 |

**(8) 对既有测试资产的影响（如实记录）**
鉴权开启后，所有 `/api/**` 调用都必须携带令牌，因此**历史 HTTP 测试脚本与性能脚本都需要先登录**：

- 主回归套件 `accept-v06.ps1` 已适配（开头登录一次并复用令牌），S88 后复跑：**6/8 通过，无一例因鉴权失败**（8 例请求均正常返回，无 401）；两例失败为**测试资产数据依赖**问题，与本步无关——V-5 硬编码的告警编号 `pt3k-00002` 已随 S74 测试数据清理删除（Mongo 现存 0 条）故返回 404；V-7 中文关键词"红外温度"在 24 小时窗口内 0 条（90 天窗口 5 条），语料随时间窗衰减。两项已登记为后续步骤修复测试资产。
- **S90 适配结果**：`accept-s50-*.ps1` 与 `accept-s33.ps1`、`accept-s86-pt8.ps1`、`export-api-contract.ps1` 全部改为"先登录再带令牌"（共享 `docker/init/auth-helper.ps1`），并已实测复跑全绿；JMeter 两个计划已加 `Authorization: Bearer ${__P(token)}` 并参数化线程/时长（配套 `docker/init/jmeter-run.ps1` 自动取令牌运行），但**尚未实测复跑**（冒烟阶段按用户要求停止 JMeter 测试）——如实标注为"已适配、待验证"。第 6 章历史性能数据均为**鉴权前**测量值（口径不变，如实标注）。


#### 5.2.9 统一异常处理与错误语义（S89）

**(1) 问题**：没有统一处理器时，参数校验失败会以 500 抛出（接口测试判定原则要求异常路径 4xx），异常栈可能随响应体返回给客户端（信息泄露），错误体结构还随异常类型漂移，前端无法统一处理。

**(2) 实现**：`common/GlobalExceptionHandler`（`@RestControllerAdvice`），把五类异常收敛为统一契约：

| 异常类型 | 状态码 | message | 附加字段 |
|---|---|---|---|
| `ResponseStatusException`（业务主动抛出） | **保留原状态码**（404 / 400 / 409 / 500） | 业务 reason 原样透出 | — |
| `MethodArgumentNotValidException`（`@Valid` 校验失败） | **400** | 参数校验失败 | `fields[]`：field / message / rejectedValue |
| `HttpMessageNotReadableException` | **400** | 请求体缺失或 JSON 格式错误 | — |
| `MissingServletRequestParameterException` | **400** | 缺少必填参数: xxx | — |
| `MethodArgumentTypeMismatchException` | **400** | 参数类型错误: xxx | — |
| `Exception`（兜底） | **500** | 服务内部错误，请稍后重试（**不吐异常栈**，细节仅落服务端日志） | — |

统一响应体：`{timestamp, status, code, error, message, path[, fields]}`；401 由鉴权拦截器直接写出，结构保持一致（见 5.2.8）。

**(3) 实测**：经网关实测六类异常均返回预期状态码与中文语义（404 设备/告警不存在、400 非法 JSON、400 校验失败带字段明细、400 非法查询参数、401 无令牌），响应 `Content-Type: application/json;charset=UTF-8`；配套单测 5 例覆盖"业务异常状态码保留 / 校验 400 带明细 / 请求体 400 / 兜底 500 不泄露异常类名与细节 / 错误体字段稳定"，`mvn test` 后端 **14/14** 全绿（明细见 6.4.3，用例见 TC037）。
...
| P-16 | 后端容器重建后，经 Nginx 访问 `/api/**` 持续 502，直连实例却正常 | Nginx 在启动时解析上游为 IP 并缓存，容器重建后 IP 变化，Nginx 仍指向旧 IP（**非**"后端启动慢"） | 部署固定动作：重建后端容器后执行 `docker compose restart nginx`；必要时对上游使用 `resolver` + 变量式 `proxy_pass` 动态解析 | **已命中并修复（S50/S63）**，见下方补充 |
| P-17 | 无状态令牌无法"登出即失效"；且双实例下内存黑名单会漏放行 | 自包含令牌的有效性只由签名与有效期决定，服务端不留状态；后端为双实例负载均衡（D-4），单实例内存黑名单无法覆盖另一实例 | 登出时把令牌编号 `jti` 写入 MongoDB 吊销表（`_id=jti` + TTL 索引），拦截器逐请求按主键校验——吊销状态必须落在共享存储而非进程内存 | **S88 已解决**（见 5.2.8(4)） |
| P-18 | 整机/仿真器重启后，重启时未完成的任务永久显示"执行中"，设备实际在默认园区巡逻 | 任务状态由回执驱动（MongoDB 持久），而执行态仅存仿真器内存；重启后 `task.command` 不重放、仿真器回默认巡逻，遗留任务永远等不到下一条回执 | 新增 `TaskReconcileService`：启动对账（宽限 60 s）+ 每 60 s 超时兜底（执行超 30 min 置 FAILED），均幂等条件更新并补回执日志（见 5.2.4(6)） | **已命中并修复（S105 / BUG-009）** |
...
**实施期实测补充（2026-09-21，S85~S88 课程四项要求）**：

- **P-17 命中（S88 权限认证）**：实现登出时确认无状态令牌无法主动作废，且双实例下必须用共享吊销表（MongoDB `auth_revoked`）——实测登出后 `auth_revoked` 文档 0→1、旧令牌复用 401，令牌跨 `backend-1`/`backend-2` 均有效；
- **鉴权开启对测试资产的连带影响（S88 新发现）**：所有 `/api/**` 调用需携带令牌，主回归套件 `accept-v06.ps1` 已适配并复跑（8 例请求无一 401，其中 2 例失败为硬编码测试数据的陈旧问题），JMeter 与 `accept-s50-*.ps1` 尚待适配——**历史性能数据为鉴权前测量值**，如实标注；
- **备份恢复比对（S87 新发现）**：仅暂停仿真的导出窗口内，后端离线检测器补发了一条 `DEVICE_OFFLINE` 告警，导致 `alarm` 集合比对出现 1 条差异；改为同时暂停全部写入方（4 仿真 + 2 后端）后 6/6 集合计数一致——备份一致性验证必须构造完全静态窗口；
- **并发压测（S86 新发现）**：逐条新开 `kafka-console-producer` 的写法因每次 JVM 启动约 0.5 s，实测只注入 1,122/3,000 条；改为每设备一个长驻生产者 + 定时喂流后注入 3,000/3,000 且丢失 0 条——**压测脚本自身的性能缺陷会伪装成被测系统的缺陷**。
- **测试资产陈旧（S90 集中修复，共 7 处）**：鉴权开启与数据清理后复跑历史脚本，暴露出大量"看起来是产品缺陷、实际是测试资产过期"的假失败：① 硬编码告警编号 `pt3k-00002` 已被 S74 清理（V-5）；② 检索关键词 24 h 窗口内语料为 0（V-7）；③ `IT009` 仍是"未实现"的硬编码结论，而能力早在 v0.6 已交付；④ 建任务脚本硬编码设备，设备忙时服务端正确返回 409 被记为失败（IT005）；⑤ 固定临时文件名 + 未校验 `hdfs dfs -get`，S82 重生成证据图后读到遗留文件导致 MD5 误报不一致（TC018）；⑥ 目录规范脚本只认 `.png`，把 S75 的 5 张 `.jpg` 现场照片误判为违规（TC019）；⑦ `TC021` 仍是硬编码 `FAIL`（TC021）。**教训**：验收脚本必须自给自足（自注入数据、唯一临时文件、按当前状态动态选样），并用 `git grep` 复核"结论性断言"是否已被后续版本推翻——否则回归报告会持续发出错误的红色信号，掩盖真实缺陷。修复后基线：`accept-s50-it` 15/0/1、`accept-s50-tc` 22/0、`accept-s50-tc2` 14/0、`accept-v06` 8/0（2026-09-21 实测）。

**实施期实测补充（2026-09-22，S105 运行期报障）**：

- **P-18 命中（S105，运行期真实报障）**：用户反馈"重启系统后，上次未完成的任务一直显示执行中，实际设备在默认园区巡逻"。排查确认三处断链——任务状态持久于 MongoDB 但由仿真器回执驱动流转、执行态仅存仿真器内存、Kafka 消费组 offset 已提交致 `task.command` 不重放；后端无启动对账、无超时兜底，幽灵任务永久悬挂。修复为"启动对账 + 超时兜底"两层清算（详见 5.2.4(6)，缺陷全记录见 6.3.3 BUG-009）。
...
| 图 5-7 | ES 索引 mapping | `GET /inspection_alarm_v3/_mapping` 输出，`geo_point` 与 keyword 字段可见 | 【待回填】 |
...
| 图 5-17 | 登录页与鉴权（S88） | 登录页界面（含**可点击的演示账号 chip**，点击自动填入）；登录后侧边栏显示当前用户与"退出登录"；未登录直接访问业务地址被重定向回登录页 | 【待回填】 |

### 5.5 本章小结

本章按 4.4 节的模块划分给出了九个模块的详细设计与关键实现。设备仿真模块采用"航点插值 + 噪声"的连续移动模型保证数据真实感，并以心跳 5 s、遥测 2 s 的双节奏解耦上报；消息接入模块通过统一生产者与"幂等消费 + 手动提交 + 死信兜底"的模板基类，把 Kafka 至少一次投递语义下的可靠性责任集中到一处；存储模块采用 WebHDFS 规避 Hadoop 客户端依赖冲突，并明确"图片本体进 HDFS、元数据进 MongoDB"；数据库模块给出了心跳 upsert、遥测批量写、2dsphere 就近派单、离线检测、任务状态原子流转与悬挂对账兜底六个关键实现；检索模块强调 `filter` 上下文与 Bulk 写入两个性能要点；网关模块给出 Nginx 配置全文及可用于现场演示负载均衡的实例标识设计；认证鉴权模块以"HMAC 自包含令牌 + 拦截器统一校验 + MongoDB 共享吊销表"实现 FR-5.7，并给出 401/403 语义边界与配置化账号的简化说明；统一异常处理模块（S89）把业务异常、参数校验失败、请求体不可解析、参数缺失/类型错误与兜底异常收敛为 `{timestamp,status,code,error,message,path[,fields]}` 一致契约，保证异常路径 4xx 而非 5xx、500 不泄露异常栈；前端模块给出了"前端不感知后端拓扑"的 API 封装、分页面刷新策略与登录态闭环（路由守卫 + 自动携带令牌 + 401 回跳）。5.3 节汇总了 18 项高频故障的现象、根因与预案，构成本章最具工程价值的部分。第 6 章将基于本章实现与第 3 章需求指标，设计并执行系统测试。
---
...
- **影像数据**：证据图由告警消费端生成入 HDFS（10,534 个 PNG，路径规范见 TC019），`alarm.snapshotPath` 记录路径；**影像元数据链路（S72 打通，BUG-008 转已实现）**——仿真每次巡逻扫描（30s）产生一张 `uav_patrol` 元数据、每次告警产生一张关联元数据（`uav_patrol`/`dog_infrared`，含 alarmId），经 `inspection.image.meta` → `biz-image-consumer` 落 `image_meta` 集合，`/api/images/meta` 可查（S72 实测：75s 内流转 8 条、告警关联 1 条）；
...
| TC015 | 消息中间件 | 消费失败进死信主题 | Kafka 正常 | 构造一条无法解析的消息并发送至告警主题 | 重试 3 次后进入 `inspection.dlq`，不阻塞后续消息消费 | S90 复跑：脚本自行注入坏消息（`this-is-not-json-…`）后 DLQ 行数 3→4 且内容可检出，后续消费不受阻。**注**：原脚本依赖队列中的遗留消息，S78 死信重放清空队列后曾误报失败——已改为自注入（测试资产修复） | 通过 |
...
| TC018 | 分布式存储 HDFS | 文件下载与 MD5 校验 | 已上传图片并有元数据 | 1. 按路径下载图片；2. 计算 MD5 与元数据比对 | MD5 完全一致 | S50 首测：API 与 HDFS 源文件 MD5 同为 `5A35D7F0…4399`（当时证据图 640×360）。S82 全量重生成 1280×720 后内容已变，**S90 复跑**：同一告警的 API 下载与 HDFS 源文件 MD5 同为 `8E1582EE…436C`，`hdfs dfs -get` 成功标志 `get=True`。**注**：原脚本用固定临时文件名且未校验 `hdfs dfs -get` 结果，重生成后曾读到上一轮遗留文件而误报不一致——已改为按运行号唯一命名并校验（测试资产修复） | 通过 |
| TC019 | 分布式存储 HDFS | 目录分区规范验证 | 已上传多张图片 | 查看 HDFS 目录结构 | 路径符合 `/inspection/{type}/{yyyy}/{MM}/{dd}/{deviceId}/` 规范 | S90 复跑：**11,541 个文件，0 违规**；扩展名实测 `{.png, .jpg}`（`.jpg` 为 S75 人工复核上传的现场照片，规范本身允许任意图片扩展名，原脚本只认 `.png` 故曾把 5 张 `.jpg` 误判为违规——已修正正则）。**注**：文件数随仿真持续产生新证据图而变化，上值为 2026-09-21 采样；S93 清理 34 个测试告警影像后为 11,507 | 通过 |
...
| TC021 | 分布式数据库 | 就近派单选中最近机器狗 | 2 台机器狗在线且空闲，位置不同 | 在靠近机器狗 A 的位置产生告警，触发派单 | 选中机器狗 A（2dsphere 查询生效） | v0.6 实现（BUG-003 回归）：2dsphere nearSphere 就近派单——V-2/V-2b 分别在 ROBOT-001/002 位置注入告警，均派给地理最近的机器狗；DONE 回执回填复核结论与红外图（D-21 仿真结论规则）。**S90**：脚本原为硬编码 `FAIL`（遗留"未实现"结论），已改为实环验证"告警→机器狗任务→复核回填"链条（实测 `task=… device=ROBOT-002 review.conclusion=CONFIRMED`），"最近"的判定仍由 V-2/V-2b 证明 | 通过（BUG-003 回归） |
...
| TC027 | Elasticsearch 检索 | 中文分词检索 | 已安装 ik 插件 | 检索中文关键词（如"围墙""人员"） | 返回包含该词的告警记录 | S63 实现（BUG-004 回归）：安装 analysis-ik 8.13.0 + 索引升版 v2（description=ik_smart）并 reindex 10,716 条；`ik_smart` 分词验证（园区/围墙/入侵/人员独立成词）；中文检索实测：关键词"人员"命中 300 条、"红外温度"命中 5 条 | 通过（BUG-004 回归） |
...
| TC032 | Web 可视化 | 告警详情证据链展示 | 存在含复核结果的告警 | 打开告警详情 | 同时展示高空原图与机器狗红外复核图及结论 | 告警详情证据链（S62 前端详情面板）：高空原图 + 机器狗红外复核图 + 结论并列展示；V-6 API 级验证 + 人工核验 | 通过 |
...
| TC035 | 权限认证 | 未登录访问受保护接口被拒 | 后端与网关正常 | 1. 不携带 `Authorization` 请求 `/api/devices`；2. 用错误口令登录；3. 空口令登录 | 1. 401（缺令牌）；2. 401（口令错误）；3. 400（参数缺失），**均不得返回 500** | S88 实测（A-1/A-2/A-3）：401 / 401 / 400，响应体含明确 `message` | 通过 |
| TC036 | 权限认证 | 登录后全链路可用与登出失效 | 演示账号可用 | 1. 登录取令牌；2. 带令牌访问 6 个业务接口；3. 登出；4. 复用旧令牌访问 | 1. 全部 200；2. 登出后旧令牌 401；3. 令牌在双后端实例均有效 | S88 实测（A-4~A-12）：6/6 接口 200、命中 backend-1 与 backend-2、`auth_revoked` 文档 0→1、旧令牌复用 401 | 通过 |
| TC037 | 异常处理 | 异常入参返回 4xx 而非 500，错误体结构统一 | 已登录（S88 鉴权后） | 1. 请求不存在的设备/告警；2. 提交非法 JSON；3. 缺必填字段；4. 传非法时间格式；5. 不携带令牌 | 1. 404；2. 400；3. **400 且返回字段明细**；4. 400；5. 401；**均不得返回 500、不得泄露异常栈** | S89 实测：404/404/400/400（含 `fields=[{field:deviceId,…}]`）/400/401，响应体统一为 `{timestamp,status,code,error,message,path[,fields]}`；配套单测 5 例（`GlobalExceptionHandlerTest`）全绿 | 通过 |
...
| FR-5.7（加分项） | 简易权限认证（登录签发令牌 + 后端统一校验 + 登出吊销） | TC035、TC036 |
| NFR-安全 | 业务接口鉴权（除登录外全部 `/api/**` 需令牌） | TC035、TC036、IT016~IT018 |
| G-2（接口正确性） | 异常入参返回明确错误码而非 500、错误体结构统一 | TC037 |
...
**实测缺陷登记（S50 + v0.6 全量回归 + S72，2026-09-14）**——按表 6-4 模板逐项登记，共 8 项：修复 7 项（BUG-001/003/004/005/006/007/008）、遗留 0 项、设计差异 1 项（BUG-002）。
...
| BUG-003 | TC021 / IT009 | 告警复核派单闭环未实现 | 一般（范围遗留） | 按设计逐接口核对，复核派单入口缺失 | 未排入 v0.5 迭代范围（2dsphere 索引已建，派单入口缺） | **v0.6 已修复**（S61/S62）：2dsphere 就近派单（空闲优先）+ POINT_REVIEW 任务 + DONE 回执回填 review（红外图 + 仿真结论规则 D-21）+ IT009 手动复核 | 回归通过：V-2/V-2b 就近派单、V-3 复核回填、V-4 复核图下载、V-5 IT009 均 PASS |
| BUG-004 | TC027 | ES 未安装 ik 中文分词插件 | 轻微 | `elasticsearch-plugin list` 输出为空 | 插件未随环境安装 | **S63 已修复**：经容器网络从 infinilabs 发布源下载 `elasticsearch-analysis-ik-8.13.0.zip`（GitHub 直链与 API 均不可用：仓库已迁移+API 限流，排错实录）→ 安装并持久化（`docker/es-plugins` bind 挂载 + `install-ik.ps1` 可复现）→ 索引升版 `inspection_alarm_v3`（description=ik_smart，严格映射）+ `_reindex` 迁移 10,796 条（中间产物 v2 因动态映射误建作废、词典目录丢失致 NPE 均修复，见 STATE 风险 #37） | 回归通过：V-7 中文检索"人员"304 条、"红外温度"5 条；`_analyze` 分词与停用词过滤正确 |
...
| BUG-008 | 6.1.2 | 影像元数据消息（`inspection.image.meta`）0 条，仿真未接入影像元数据上报 | 轻微（设计差异） | 数据准备核对 topic offset 为 0 | 仿真侧影像上报路径未实现 | **S72 已实现**：仿真巡逻扫描（30s）与告警均产生影像元数据（含 alarmId 关联）→ `biz-image-consumer` 落 `image_meta` 集合 → `/api/images/meta` 查询接口 | 回归通过：S72 实测 topic 流转、75s 内入库 8 条、告警关联元数据 1 条；TC005/TC018/TC019 覆盖证据图链路不变 |
| BUG-009 | TC033（延伸场景） | 整机/仿真器重启后，重启时未完成的任务永久显示"执行中"（实测存量一条悬挂 4 小时以上），设备实际在默认园区巡逻；仅后端重启不复现 | 严重 | 用户报障 → 复现条件确认为"重启瞬间有任务处于 DISPATCHED/RUNNING"→ 代码走查：任务状态由 `task.log` 回执驱动（MongoDB 持久），而 `currentTaskId`/`taskQueue` 仅存仿真器 JVM 内存 → 再核对 Kafka 消费组 offset 已提交、`task.command` 不重放；后端无启动对账、无超时清理 | 回执驱动状态机与内存态执行器在重启时断链：持久层保留 RUNNING，内存态清零且轨迹回默认巡逻，幽灵任务永远等不到下一条回执 | **S105 已修复**：新增 `TaskReconcileService` 两层兜底——① 启动对账：应用就绪即清算派发超宽限期（60 s）的未终结任务；② 超时兜底：每 60 s 扫描执行超 30 min 的任务；均条件更新置 FAILED + finishTime + 补 FAILED 回执日志（详见 5.2.4(6)） | E2E 回归：AREA_COVER 执行中重启 backend×2 + uav-sim-1，后端就绪即清算 FAILED 且 finishTime 回填 ✓；新镜像首启清算历史悬挂任务（挂 4 h+）✓；重启后新派任务 RUNNING→DONE 正常 ✓ |

> 说明：S50 期间测试脚本自身缺陷（PS 5.1 引号传递、计数提取、批号幂等）经 4 轮修正，属测试资产缺陷而非产品缺陷，过程记录于 STATE 风险 #16 族。

**缺陷补充登记（2026-09-22，S105 运行期报障）**——BUG-009 为系统运行期真实报障（用户反馈"重启后上次未完成任务永久执行中"），按表 6-4 模板补登记。累计 9 项：修复 8 项（BUG-001/003/004/005/006/007/008/009）、设计差异 1 项（BUG-002）、遗留 0 项。
...
| IT016 | `/api/auth/login` | POST | 正确的用户名与口令 | 200，返回令牌、角色与过期时间（`token`/`role`/`expiresAt`） |
| IT017 | `/api/auth/login` | POST | 错误口令 / 空口令 | 401 / 400，响应体含明确 `message`，**不得返回 500** |
| IT018 | `/api/devices` | GET | 无令牌 / 篡改令牌 / 伪造签名 / 登出后复用 | 四种情形均 401，语义可区分，不得返回 500 |

**接口测试实测结果（2026-09-12，脚本 `docker/init/accept-s50-it.ps1` 可复现）**：15 例中 **13 通过、0 不通过、2 未实现**。

**接口测试复跑结果（2026-09-21，S90 测试资产修复后）**：脚本共 **16 项断言（IT001~IT015 + IT009b）→ 15 通过、0 不通过、1 未实现（仅 IT001 设计差异）**。本步修复的三处测试资产陈旧问题：① **IT009** 原脚本硬编码"review dispatch not implemented"的 `UNIMPL` 结论，而该能力早在 S61/v0.6 已实现——改为动态选取未处置告警实测复核（200，`status=FALSE_ALARM`、`reviewerDeviceId=MANUAL`）；② **IT005** 原脚本硬编码向 `ROBOT-001` 建任务，设备恰好忙时服务端正确返回 **409**（单设备单活动任务）却被记为失败——改为先查询空闲在线设备再建任务；③ 新增 **IT009b**：非法复核结论必须返回 **400** 而非 500。

**接口测试实测结果（鉴权接口，2026-09-21，脚本 `docker/init/accept-s88.ps1` 可复现）**：IT016~IT018 三例**全部通过**——IT016 登录 200（令牌长 204、`role=ADMIN`、返回 `expiresAt`）；IT017 错误口令 401 / 空口令 400；IT018 无令牌、篡改令牌、伪造签名、登出后复用四种情形均 401。同一脚本还验证了令牌跨双后端实例有效（16 次请求命中 backend-1 与 backend-2）与前端产物含登录页，13 例断言全部通过。

- IT001（注册接口）：**未实现——设计差异**。设备注册由心跳 upsert 驱动（TC001 已验证注册语义），无独立注册端点；
- IT009（告警复核接口）：**v0.6 已实现**（S61，BUG-003 回归）——`POST /api/alarms/{alarmId}/review` 实测：合法结论 200（status/review 更新并同步 ES）、非法结论 400；
- 其余 14 例实测通过，关键证据：IT004 404 无 500、IT005 201、IT006 400（`@Valid` 校验）、IT007 取消后状态 CANCELLED、IT011 非法时间 400（ES 异常已转译）、IT013 证据图按告警编号下载成功、IT015 响应头实例号可见。
...
# S88 权限认证：登录取令牌，后续请求统一携带（未携带一律 401）
TOKEN=$(curl -s -X POST http://localhost:8080/api/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"admin","password":"admin123"}' | jq -r .token)
curl -s -o /dev/null -w '%{http_code}\n' http://localhost:8080/api/devices                      # 401（无令牌）
curl -s -o /dev/null -w '%{http_code}\n' -H "Authorization: Bearer $TOKEN" http://localhost:8080/api/devices   # 200
curl -s -X POST -H "Authorization: Bearer $TOKEN" http://localhost:8080/api/auth/logout | jq .
curl -s -o /dev/null -w '%{http_code}\n' -H "Authorization: Bearer $TOKEN" http://localhost:8080/api/devices   # 401（已登出）
```

**接口测试的判定原则**：不仅看"正常路径返回 200"，更要看**异常路径是否返回 4xx 而非 5xx**。把参数校验缺失导致的 500 当作通过，是接口测试最常见的自欺。本项目通过 `@Valid` + `GlobalExceptionHandler`（`@RestControllerAdvice`，S89 落地，详见 5.2.9）统一保证：业务异常保留原状态码与 message、参数校验失败返回 **400 + 逐字段明细**、兜底异常返回 **500 且不吐异常栈**。

**统一错误响应体实测（2026-09-21，S89 部署后经网关实测）**：

```json
{"timestamp":"2026-09-21T04:06:05.696Z","status":404,"code":404,"error":"NOT_FOUND",
 "message":"device not found: NOPE-999","path":"/api/devices/NOPE-999"}
```

| 场景 | 实测状态码 | 响应关键字段 |
|---|---|---|
| `GET /api/devices/NOPE-999`（不存在的设备） | **404** | `message=device not found: NOPE-999` |
| `GET /api/alarms/NOPE-999`（不存在的告警） | **404** | `message=alarm not found: NOPE-999` |
| `POST /api/tasks` 请求体非法 JSON | **400** | `message=请求体缺失或 JSON 格式错误` |
| `POST /api/tasks` 缺少必填字段 | **400** | `fields=[{field:deviceId, message:"deviceId 不能为空"}]`（**含字段明细**） |
| `POST /api/search/alarms` 非法时间格式 | **400** | `message=查询参数错误: …`（ES 异常已转译为业务语义） |
| 无令牌访问任意业务接口（S88 鉴权） | **401** | `Content-Type: application/json;charset=UTF-8`，`{code,error,message}` |

#### 6.4.3 单元测试（S77 建立，S89 扩充）

| 模块 | 用例数 | 覆盖内容 | 结果 |
|---|---|---|---|
| 后端 `common/JsonTest` | 2 | 消息 JSON 序列化往返（含中文描述，UTF-8 口径） | 通过 |
| 后端 `storage/PathBuilderTest` | 3 | HDFS 分区路径拼接规范 | 通过 |
| 后端 `task/TaskServiceTest` | 4 | 任务状态流转与取消语义（Mockito 隔离持久层） | 通过 |
| 后端 `common/GlobalExceptionHandlerTest`（S89 新增） | 5 | 业务异常状态码保留、校验失败 400 带字段明细、请求体不可解析 400、兜底 500 不泄露异常类名与细节、错误体字段稳定 | 通过 |
| 仿真 `TelemetryGeneratorTest` | 3 | 轨迹连续性、坐标范围、电量单调性 | 通过 |
| 仿真 `GroundNetworkTest` | 3 | 空地网络切换与连通性判定 | 通过 |
| **合计** | **20** | 后端 14 + 仿真 6 | **20/20 通过（`mvn test`，2026-09-21 复跑）** |

> 单元测试的价值在 S89 立刻兑现：`GlobalExceptionHandlerTest` 在首次运行时就暴露了"MockMvc 默认以 ISO-8859-1 解码响应体导致中文断言恒失败"的测试写法缺陷——这是**测试自身的缺陷**，与"把测试写绿"相比，先证明测试会红更有意义。
...
| PT008 | **多设备并发上报**（S86 补测） | 4 台设备（2 无人机 + 2 机器狗）**各自 12.5 msg/s 并发上报，持续 60 s**（合计 ≈50 msg/s，共 3,000 条） | 零丢失、消费 LAG 回落、排水时延 | 3,000 条全部落库（分设备各 750 条），LAG 峰值 0、排水 13.7 s |
...
|---|---|---|---|---|---|
| 功能测试（TC001~TC034） | 34 | 34 | 0 | 0 | 100% |
| 接口测试（IT001~IT015） | 15 | 14 | 0 | 1（IT001 设计差异） | 93.3%（已实现范围 100%） |
| 接口测试复跑（+IT009b，S90） | 16 | 15 | 0 | 1（IT001 设计差异） | 93.8%（已实现范围 100%） |
| 性能测试（PT001~PT007） | 7 | 7 | 0 | 0 | 100% |
| 性能补测（PT008 多设备并发上报，S86） | 1 | 1 | 0 | 0 | 100% |
| 权限认证功能（TC035~TC036，S88） | 2 | 2 | 0 | 0 | 100% |
| 权限认证接口（IT016~IT018，S88） | 3 | 3 | 0 | 0 | 100% |
| 异常语义功能（TC037，S89） | 1 | 1 | 0 | 0 | 100% |
| 单元测试（后端 14 + 仿真 6，S77/S89） | 20 | 20 | 0 | 0 | 100% |

> 唯一未实现项 IT001 为设计差异（设备注册由心跳 upsert 驱动，无独立注册端点，已登记 BUG-002）。v0.6 迭代将 S50 登记的两项遗留（BUG-003 复核派单、BUG-004 ik 分词）全部实现并回归通过；**S88 落地 FR-5.7 简易权限认证（新增 5 例断言全过：TC035/TC036、IT016~IT018）**；不通过数全程为 0。
...
| 持续上报能力 | ≥ 50 msg/s 零丢失 | **4 台设备并发上报 50 msg/s（3,000 条）零丢失、排水 13.7 s（PT008）**；批量 500 条注入增量 +556（含同期自然上报），LAG 回落 0 | 达标 |
...
1. **是否达标**：表 6-8 七项指标全部达标，且关键指标裕量显著（检索 P95 仅为目标值的 5.6%、网关吞吐为目标值的 13 倍）。功能测试 34/34 通过、0 失败；接口测试 14/15 通过、0 失败（IT001 为设计差异）。不通过数全程为 0。
...
系统满足第 3 章全部量化需求与验收标准：遥测端到端时延、检索 P95、持续上报零丢失、网关吞吐、负载均衡、限流、故障转移、数据一致性均实测达标（G-1~G-8 全部满足）。测试期间发现并修复 4 项真实缺陷（BUG-001 限流缺失、BUG-005 索引未落地、BUG-006 ES 总数截断、BUG-007 并发取消竞态），均为"设计有、实现缺"或"默认行为陷阱"类问题，全部回归通过；**v0.6 迭代将 S50 登记的两项遗留全部实现并回归通过：BUG-003 复核派单闭环（TC021/IT009/TC032 转绿）、BUG-004 ik 中文分词（TC027 转绿，索引升版 v3）**；**S72 将 BUG-008 影像元数据链路打通（image_meta 集合流转）**；**S85~S88 补齐课程的"简单权限认证 / 任务日志完整归档 / 多设备并发上报 / 备份恢复演示"四项要求：日志按任务可查（S85）、四设备并发 3,000 条零丢失（S86/PT008）、备份恢复 6 集合计数一致（S87）、FR-5.7 简易权限认证 13 例全过（S88）**；唯一设计差异 BUG-002（注册端点由心跳驱动）不阻塞验收。测试数据规模按设计补齐（10,000 条告警注入），性能结论在 10k 数据规模下成立。
...
本章完成了"环境 → 目标 → 功能测试 → 接口测试 → 性能测试 → 结果分析 → 优化"的完整闭环。**v0.6 迭代后功能测试 34/34、性能测试 7/7 全部通过，接口测试 14/15（唯一未实现 IT001 为设计差异）**：检索 P95 28 ms、网关吞吐 2,632 rps、遥测批量零丢失、限流与故障转移均实测生效。测试不是"跑一遍都绿"：S50 实际暴露并修复了 4 项真实缺陷（限流缺失、时序集合缺索引、ES 总数截断、并发取消竞态），其中缺索引问题经过了"888.8 ms → 聚合方案劣化 1,229.7 ms → 复合索引 36.3 ms"的完整测量-定位-优化-复测路径，以数据驳回了想当然的优化方案；10,000 条告警的数据规模准备则验证了 6.1.1"数据过少结论无意义"的警告——ES 总数截断缺陷仅在数据超万后暴露。**v0.6 迭代兑现了 S50 登记的全部两项遗留：复核派单闭环（BUG-003，2dsphere 就近派单+红外复核图回填+IT009）与 ik 中文分词（BUG-004，索引升版 v2），v0.6 验收 8/8 回归通过（TC021/TC027/IT009/TC032 全部转绿）**；2 项设计差异（注册端点、影像元数据主题）均有登记。第 2~6 章至此闭环：第 3 章定需求与指标，第 4 章定架构与数据，第 5 章定实现与故障预案，第 6 章以实测数据验证需求达成度并反哺优化方向（6.6.3 第 4 点的架构启示将写入第 7 章后续改进）。
...
本项目完成了"无人机-机器狗空地协同巡检集成平台"从设计到测试交付的全过程，工作按阶段总结如下：

1. **环境与基础**：以 Docker 编排六大组件（HDFS 3.3.6 / MongoDB 6.0 / Kafka 4.0.0 / ES 8.13 + Kibana / Nginx 1.25），产出可复现的初始化脚本（副本集、8 主题、限流与负载均衡配置）；踩坑 39 项全部实录入 STATE 风险档案（如 envtoconf 缺陷、Kibana basePath 反代、Kafka 镜像更换等），形成可复用的排错知识。
2. **核心链路**：仿真侧实现多模式轨迹、优先级任务队列、电量循环与故障注入；平台侧实现心跳 upsert、告警三写（HDFS 证据图 + MongoDB 权威 + ES 检索副本）、指令下行与回执驱动状态机、离线检测、组合检索与聚合统计。
3. **业务闭环（v0.6）**：兑现"高空发现 → 就近派单 → 抵近复核 → 证据回填"闭环——2dsphere 就近派机器狗、POINT_REVIEW 任务关联告警、DONE 回执回填红外复核图与结论、IT009 手动复核兜底；安装 ik 插件并升版索引实现中文分词检索。
4. **测试与优化**：S50 全量测试（功能 34/34、接口 14/15、性能 7/7），数据规模按设计补齐 10,000 条告警；测试暴露并修复 4 项真实缺陷（限流缺失、时序集合缺索引、ES 总数截断、并发取消竞态），完成"888.8 ms → 36.3 ms"的测量-定位-优化-复测全路径；缺陷登记 BUG-001~008 全部闭环（修复 6、设计差异 2）。

量化成果（均实测）：检索 P95 28 ms、网关吞吐 2,632 rps、遥测批量零丢失、负载均衡 50/50、限流 200 突发 158×503、复核闭环验收 8/8。全过程 60 余次提交、5 个里程碑（v0.1~v0.5），每一步验收结论与排错实录均留痕于 PLAN/STATE 治理文件。

### 7.2 不足与改进方向

如实记录当前不足（均已列入后续计划 S68~S72）：

| 编号 | 不足 | 影响 | 改进方向（计划） |
|---|---|---|---|
| 1 | Mongo↔ES 对账补偿未自动化（D-5 承诺了"对账补偿"，目前仅日志） | ES 副本极端情况下可能静默缺文档 | S70：定时对账 + 差集补写 + 结果可查 |
| 2 | ~~影像元数据链路未接通（BUG-008 设计差异）~~ **S72 已打通** | 已解决：仿真产生元数据 → 消费入库 → 接口可查 | 完成（2026-09-14） |
| 3 | 仿真任务队列在重启后丢失（内存队列，风险 #36③） | 重启产生孤儿任务，需人工取消 | 队列持久化或重启补偿机制 |
| 4 | 复核结论为仿真规则（D-21：哈希 70/30），非真实识别模型 | 结论无实际视觉依据 | 接入真实识别模型或规则可配置化 |
| 5 | ES 深分页未防护（from+size 上限 10,000，与 BUG-006 同类陷阱） | 深翻页可能 500 | S71：超限 400 + 前端截断提示 |
| 6 | 手动复核仅有接口（IT009），前端无按钮 | 人工处置路径不完整 | S68：详情面板复核按钮 |
| 7 | 设备轨迹未可视化（时序数据已具备） | 演示缺少"轨迹回放"亮点 | S69：轨迹查询接口 + 地图轨迹线 |
| 8 | 版本库仅本地，无异地备份 | 数据丢失风险 | S60 候选：git bundle / 远端仓库 |

此外，第 6.6.3 节"架构启示"的四条结论将作为后续迭代的设计红线：时序集合必须建 `{deviceId, ts}` 复合索引；ES 分页/统计必须显式 `track_total_hits`；索引迁移必须先建映射再 reindex（动态映射误建事故，风险 #37）；容器重建后必须刷新 Nginx 上游解析。

### 7.3 展望

1. **规模化**：从 4 台仿真设备扩展到真实设备接入与多园区部署，容器编排向 K8s 演进；
2. **智能化**：接入真实视觉识别模型（高空目标检测、红外热成像判读），替换仿真结论规则，使"复核结论"具备真实依据；
3. **可靠性**：落地对账补偿与队列持久化，补齐"至少一次投递"语义下的完整兜底；
4. **可视化**：Kibana 正式仪表盘、设备轨迹回放与告警大图预览，进一步提升操作体验。
