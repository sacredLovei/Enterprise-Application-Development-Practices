...
| TC019 | 分布式存储 HDFS | 目录分区规范验证 | 已上传多张图片 | 查看 HDFS 目录结构 | 路径符合 `/inspection/{type}/{yyyy}/{MM}/{dd}/{deviceId}/` 规范 | S90 复跑：**11,541 个文件，0 违规**；扩展名实测 `{.png, .jpg}`（`.jpg` 为 S75 人工复核上传的现场照片，规范本身允许任意图片扩展名，原脚本只认 `.png` 故曾把 5 张 `.jpg` 误判为违规——已修正正则） | 通过 |
